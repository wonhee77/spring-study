...
### 요약
웹 화면을 이용해서 테스트를 진행한다면 필요한 기능이 다 만들어져야 하고, 에러가 발생하더라도 어디서 에러가 발생했는지 직접 다 찾아야 한다.  
...
### 요약
자신이 만들이 않은 프레임워크나 다른 팀에서 개발한 라이브러리 등에 대해서도 테스트를 작성해야한다. 이런 테스트를 학습 테스트라고 한다.  

학습 테스트의 장점
- 다양한 조건에 따른 기능을 손쉽게 확인해볼 수 있다.  
다양한 조건에 따라 동작하는 것을 확인하려면 수동으로 값을 입력하거나 코드를 계속 수정해야하지만 테스트를 만들면 자동화된 코드로 빠르게 결과를 확인할 수 있다.  
  
- 학습 테스트 코드를 개발 중에 참고할 수 있다.  
수동으로 기능을 확인하면 코드가 수정되기 때문에 최종 결과물만 남아있으나 학습테스트는 다양한 테스트에 대한 기록이 남아 있다.  
  또한 테스트 코드를 실제 개발에서 샘플로 쓸 수도 있으며 다른 팀원들이 테스트를 통해 해당 기능을 확인할 수 있다.  
  
- 프레임워크나 제품을 업그레이드할 때 호환성 검증을 도와준다.  
제품이 업그레이드 됨에 따라 기존의 기능들이 조금씩 변경될 수 있다. 테스트 코드를 만들어 놓으면 버전이 올라가더라도 변화에 대한 탐지가 가능하다.  
  
- 테스트 작성에 대한 좋은 훈련이 된다.  

버그 테스트는 코드에 오류가 있을 때 그 오류를 가장 잘 드러내줄 수 있는 테스트를 말한다.  
오류가 발견되면 테스트코드를 통해 먼저 그 버그를 확인을 하자. 코드를 수정한 뒤 테스트가 성공하면 버그가 수정한 것이라고 보아도 된다.  
