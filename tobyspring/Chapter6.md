...
포인트컷 표형식을 지원하는 포인트컷을 적용하려면 AspectJExpressionPointcut 클래스를 사용하면 된다. AspectJExpressionPointcut은 클래스와 메소드의 선정 알고리즘은  
...
리턴 값의 타입에 대한 제한을 없애고 어떤 리턴 타입을 가졌든 상관없이 선정하려면 * 와일드카드를 쓰면된다.
파라미터의 개수와 타입을 무시하려면 ()안에 ..을 넣어준다.
모드 메소드를 다 허용하려면 메소드명에 와일드카드 *를 넣어준다.

#### 포인트컷 표현식을 이용하는 포인트컷 적용
AspectJ 포인트컷 표현식은 메소드의 시그니처를 비교하는 방식인 execution 이외에도 빈 이름으로 비교하는 bean()이 있다.
bean(*Service) 라고 하면 Service와 매칭되는 빈 이름을 가진 빈을 매칭한다.  
